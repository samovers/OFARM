...
REGISTRY = IMPL / 'OFARM_external_evidence_decision_registry_v0_2.json'
PACKET = IMPL / 'OFARM_External_Evidence_Decision_and_Disposition_Packet_v0_2.md'
...
TEMPLATE = IMPL / 'OFARM_external_evidence_decision_record_template_v0_2.json'
INTAKE_RESULTS = IMPL / 'OFARM_external_evidence_intake_results_v0_4.json'
SNAP = IMPL / 'OFARM_post_hardening_readiness_snapshot_v0_7.json'
RESULTS_OUT = IMPL / 'OFARM_external_evidence_decision_results_v0_2.json'
...
LANES = {
    'runtimeSurfaceReviewDecisions': {
        ...
        'positive': {'QUALIFY_AS_GOVERNED_RUNTIME_SURFACE_LIVE_EVIDENCE'},
        'expected_positive_claim_kind': 'GOVERNED_RUNTIME_SURFACE_LIVE_DEPLOYMENT_EVIDENCE',
    },
    'partnerOutputReviewDecisions': {
        ...
        'positive': {'QUALIFY_AS_PARTNER_OUTPUT_SUPPORT_EVIDENCE'},
        'expected_positive_claim_kind': 'PARTNER_OUTPUT_SUPPORT_TELEMETRY',
    },
    'sameStandardBridgeReviewDecisions': {
        ...
        'positive': {'COUNT_TOWARD_SAME_STANDARD_BRIDGE_PROMOTION_GATE'},
        'expected_positive_claim_kind': None,
    },
}
...
def is_placeholder(value: Any) -> bool:
    if isinstance(value, str):
        return value.startswith('replace-with-') or 'FILL_WITH_REAL' in value or 'YYYY-MM-DD' in value
...
def qualifying_artifact_refs(intake_results: dict[str, Any]) -> set[str]:
    out: set[str] = set()
    for payload in intake_results.get('artifactDiscovery', {}).values():
        for rel in payload.get('qualifyingFiles', []):
            rel_path = f'04_implementation_and_conformance/{rel}' if not rel.startswith('04_implementation_and_conformance/') else rel
            out.add(rel_path)
    return out


def classify_decision(path: Path, spec: dict[str, Any], intake_results: dict[str, Any], qualifying_refs: set[str]) -> dict[str, Any]:
    payload = load_json(path)
    reasons: list[str] = []
    for key in ['templateOnly', 'reviewedArtifactRef', 'reviewedArtifactDigest', 'reviewedQualificationClaimKind', 'decisionLane', 'decision', 'reviewedAt', 'reviewerPartyRef', 'reviewerRoleRef', 'linkedIntakeResultsRef']:
    ...
    if is_placeholder(payload.get('reviewedArtifactRef')):
        reasons.append('PLACEHOLDER:reviewedArtifactRef')
    if not isinstance(payload.get('reviewedArtifactRef'), str) or not payload.get('reviewedArtifactRef', '').startswith(spec['evidence_prefix']):
        ...
        artifact_path = IMPL.parent / payload['reviewedArtifactRef'].replace('04_implementation_and_conformance/', '')
    ...
    for field in ['reviewedArtifactDigest', 'reviewedQualificationClaimKind', 'reviewerPartyRef', 'reviewerRoleRef']:
        if is_placeholder(payload.get(field)):
            reasons.append(f'PLACEHOLDER:{field}')

    positive_decision = payload.get('decision') in spec['positive']
    if payload.get('decision') == 'COUNT_TOWARD_SAME_STANDARD_BRIDGE_PROMOTION_GATE':
        if not payload.get('candidatePairId') or is_placeholder(payload.get('candidatePairId')):
            reasons.append('MISSING_CANDIDATE_PAIR_ID')

    if artifact is not None:
        auth = artifact.get('authenticityEnvelope') or {}
        claim = artifact.get('qualificationClaim') or {}
        art_digest = auth.get('artifactDigest')
        if art_digest is not None and payload.get('reviewedArtifactDigest') != art_digest:
            reasons.append('ARTIFACT_DIGEST_MISMATCH')
        if claim.get('claimKind') is not None and payload.get('reviewedQualificationClaimKind') != claim.get('claimKind'):
            reasons.append('ARTIFACT_CLAIM_KIND_MISMATCH')
        if positive_decision:
            if payload.get('reviewedArtifactRef') not in qualifying_refs:
                reasons.append('POSITIVE_DECISION_ARTIFACT_NOT_IN_QUALIFYING_INTAKE_RESULTS')
            if auth.get('repositoryAuthored') is not False:
                reasons.append('POSITIVE_DECISION_REQUIRES_NON_REPOSITORY_AUTHORED_ARTIFACT')
            expected_claim = spec['expected_positive_claim_kind']
            if expected_claim is not None and claim.get('claimKind') != expected_claim:
                reasons.append('POSITIVE_DECISION_CLAIM_KIND_MISMATCH')
...
def main() -> int:
    ...
    qualifying_refs = qualifying_artifact_refs(intake_results)
    ...
    expected_paths = [
        PACKET, REGISTRY, REVIEWER_PACKET, REVIEWER_CHECKLIST, AUTH_NOTE, TEMPLATE, INTAKE_RESULTS,
    ...
    posture_ok = registry.get('overall') == 'PREPARED_FOR_AUTHENTICITY_AWARE_ACCOUNTABLE_EXTERNAL_EVIDENCE_DECISIONS'
    results['decisionLaneChecks']['registry-overall-posture'] = 'PASS' if posture_ok else 'FAIL'
    if not posture_ok:
    ...
    results['decisionLaneChecks']['snapshot-phase'] = 'PASS' if snapshot_ok else 'FAIL'
    if not snapshot_ok:
        results['failingChecks'].append('snapshot phase mismatch')
    intake_ok = intake_results.get('overall') == 'PASS_WITH_LIMITATIONS'
    results['decisionLaneChecks']['intake-runner-overall'] = 'PASS' if intake_ok else 'FAIL'
    if not intake_ok:
    ...
    for key, spec in LANES.items():
        ...
        classified = [classify_decision(path, spec, intake_results, qualifying_refs) for path in files]
