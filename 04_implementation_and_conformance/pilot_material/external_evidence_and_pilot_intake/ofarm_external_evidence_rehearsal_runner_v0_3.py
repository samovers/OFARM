...
RUNTIME = REHEARSAL / 'runtime_surface_release_lane' / 'OFARM_runtime_surface_live_deployment_evidence_rehearsal_v0_2.json'
PARTNER = REHEARSAL / 'partner_output_channels' / 'OFARM_runtime_surface_partner_output_telemetry_rehearsal_v0_2.json'
README = REHEARSAL / 'README.md'
INTAKE_RESULTS = IMPL / 'OFARM_external_evidence_intake_results_v0_4.json'
DECISION_RESULTS = IMPL / 'OFARM_external_evidence_decision_results_v0_2.json'
OUT = IMPL / 'OFARM_external_evidence_rehearsal_results_v0_3.json'
...
def has_placeholder(value: Any) -> bool:
    if isinstance(value, str):
        return value.startswith('replace-with-') or 'YYYY-MM-DD' in value or value.startswith('<')
...
def main() -> int:
    ...
    decisions = load_json(DECISION_RESULTS)

    checks: dict[str, str] = {}
    issues: list[str] = []

    for path in [README, RUNTIME, PARTNER, INTAKE_RESULTS, DECISION_RESULTS]:
    ...
    def auth_expect(payload: dict[str, Any], field: str, expected: Any, label: str) -> None:
        ok = isinstance(payload.get('authenticityEnvelope'), dict) and payload['authenticityEnvelope'].get(field) == expected
        checks[label] = 'PASS' if ok else 'FAIL'
        if not ok:
            issues.append(f'{label} expected {expected!r}')
    ...
    auth_expect(runtime, 'repositoryAuthored', True, 'runtime rehearsal explicitly repo-authored')
    auth_expect(runtime, 'sourceRealityClass', 'REHEARSAL', 'runtime rehearsal source reality class')
    ...
    auth_expect(partner, 'repositoryAuthored', True, 'partner rehearsal explicitly repo-authored')
    auth_expect(partner, 'sourceRealityClass', 'REHEARSAL', 'partner rehearsal source reality class')
    ...
    decisions_still_zero = decisions.get('summary', {}).get('packageStillMissingAccountableReviewDecisions') is True
    checks['decision lane still has zero accountable review decisions'] = 'PASS' if decisions_still_zero else 'FAIL'
    if not decisions_still_zero:
        issues.append('decision lane unexpectedly reports accountable review decisions')

    results = {
        'evaluatedAt': now_iso(),
        'rehearsalRoot': '04_implementation_and_conformance/pilot_material/pilot_intake_rehearsal/',
        'rehearsalArtifacts': [
            '04_implementation_and_conformance/pilot_material/pilot_intake_rehearsal/runtime_surface_release_lane/OFARM_runtime_surface_live_deployment_evidence_rehearsal_v0_2.json',
            '04_implementation_and_conformance/pilot_material/pilot_intake_rehearsal/partner_output_channels/OFARM_runtime_surface_partner_output_telemetry_rehearsal_v0_2.json'
        ],
        'checks': checks,
        'summary': {
            ...
            'reviewDecisionsAddedByRehearsal': 0,
            'liveDropZonesRemainUnevidenced': live_still_zero,
            'reviewDecisionLanesRemainEmpty': decisions_still_zero,
        },
        'limitations': [
            'Rehearsal packets remain non-qualifying by design.',
            'Same-standard bridge rehearsal artifacts are intentionally absent.',
            'A real deployment artifact and a real review decision are still required before the readiness gate can change materially.'
        ],
