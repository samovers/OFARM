# OFARM post-hardening readiness gate memo v0.7

Date: 2026-04-20
Status: active supporting implementation artifact
Scope: refreshed readiness recommendation after adding authenticity-aware qualification to the external evidence lane

Reviewed package:
- RC2.1 baseline (`00_active_baseline/`)
- companion artifacts and accepted RFC closure set already carried forward in the v0.6 checkpoint
...
It removes one more delivery ambiguity: the package now has a current authenticity and qualification rule for the first real external evidence packet.
...
### 4. External evidence collection is now authenticity-gated as well as structurally gated
`OFARM_external_evidence_intake_results_v0_4.json` is expected to remain `PASS_WITH_LIMITATIONS` and confirm that the package still has:
...
- one explicit authenticity and qualification note

`OFARM_external_evidence_decision_results_v0_2.json` is expected to remain `PASS_WITH_LIMITATIONS` and confirm that the package now also has:
- one authenticity-aware decision/disposition packet
...
Current qualifying evidence counts remain zero, and current accountable decision counts remain zero, but the first-packet review path is now less vulnerable to rehearsal-copy drift or optimistic qualification.
...
The package now has a current runtime-surface live-evidence packet, a current handoff kit, a current authenticity note, a reviewer disposition lane, and a rehearsal lane.
...
The package now has an explicit partner-output telemetry intake lane and a reviewer-side disposition lane.
...
5. rerun `ofarm_external_evidence_intake_runner_v0_4.py`
6. record any accountable decision under `live_evidence_decisions/`
7. rerun `ofarm_external_evidence_decision_runner_v0_2.py`
