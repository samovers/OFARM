...
STRONG_FORBIDDEN = {
    'use:farm-truth','use:current-state','use:compliance-fact','use:mission-authority','use:mission-dispatch','DISPATCH_AUTHORITY','MISSION_DISPATCH',
...
def semantic_checks(records):
    ...
    for o in by_type(records,'IntelligenceOutputQualification'):
        ...
        needed={'use:farm-truth','use:current-state','use:compliance-fact','use:mission-authority','use:mission-dispatch','use:model-deployment','use:automatic-execution'}
    ...
    # CP14 Phase 7.2 cross-record policy, disclosure, and federated-use hardening
    # Active grants must resolve to active policy, active boundary, and active recipient-use constraints.
    for g in by_type(records,'FarmIntelligenceShareGrant'):
        if g.get('grantState') == 'ACTIVE':
            pol=ids.get(g.get('sharePolicyRef'))
            if not pol or pol.get('policyState') != 'ACTIVE':
                fail(f"{g['artifactId']} active grant references missing/non-active share policy", errors)
            b=ids.get(g.get('boundaryRef'))
            if not b or b.get('boundaryState') != 'ACTIVE':
                fail(f"{g['artifactId']} active grant references missing/non-active boundary", errors)
            for ruc_ref in g.get('recipientUseConstraintRefs',[]):
                ruc=ids.get(ruc_ref)
                if not ruc or ruc.get('constraintState') != 'ACTIVE':
                    fail(f"{g['artifactId']} active grant references missing/non-active recipient-use constraint {ruc_ref}", errors)
            # if policy specifies required constraints, they must be covered by the grant
            if pol:
                required=set(pol.get('requiredRecipientUseConstraintRefs',[]))
                if required and not required.issubset(set(g.get('recipientUseConstraintRefs',[]))):
                    fail(f"{g['artifactId']} active grant does not cover share-policy recipient-use constraints", errors)

    # Policy allowed/prohibited overlap checks beyond generic allowedUseClasses/prohibitedUseClasses.
    for pol in by_type(records,'FarmIntelligenceSharePolicy'):
        if overlaps(pol.get('allowedPurposeClasses'), pol.get('prohibitedPurposeClasses')):
            fail(f"{pol['artifactId']} allowedPurposeClasses/prohibitedPurposeClasses overlap", errors)
        if pol.get('policyState') == 'ACTIVE' and (not pol.get('revocationPolicyRef') or not pol.get('retentionPolicyRef')):
            fail(f"{pol['artifactId']} active share policy lacks retention/revocation policy", errors)
    for pol in by_type(records,'DerivativeUsePolicy'):
        if overlaps(pol.get('permittedDerivativeClasses'), pol.get('prohibitedDerivativeClasses')):
            fail(f"{pol['artifactId']} permittedDerivativeClasses/prohibitedDerivativeClasses overlap", errors)
    for pol in by_type(records,'TrainingUsePolicyBinding'):
        if overlaps(pol.get('allowedTrainingPurposeClasses'), pol.get('prohibitedTrainingPurposeClasses')):
            fail(f"{pol['artifactId']} allowedTrainingPurposeClasses/prohibitedTrainingPurposeClasses overlap", errors)

    # Output qualification disclosure/authority hardening.
    for o in by_type(records,'IntelligenceOutputQualification'):
        allowed=set(o.get('allowedUseClasses',[]))
        if 'use:mission-dispatch' in allowed or 'DISPATCH_AUTHORITY' in allowed or 'MISSION_DISPATCH' in allowed:
            fail(f"{o['artifactId']} output qualification allows mission dispatch/dispatch authority", errors)
        if o.get('qualificationState') != 'ACTIVE' and (allowed or o.get('outputDisposition') in {'REGIONAL_ALERT_ADVISORY','BENCHMARK_ADVISORY','SHARE_PACKAGE','FEDERATED_RECEIPT','MODEL_IMPROVEMENT_ADVISORY'}):
            fail(f"{o['artifactId']} output-bearing qualification is not ACTIVE", errors)
        if 'use:public-disclosure' in allowed or o.get('disclosureClass') == 'PUBLIC':
            if not o.get('publicAuthorityBasisRef') and not o.get('sharingGrantRefs'):
                fail(f"{o['artifactId']} public disclosure lacks sharing grant or public-authority basis", errors)
            for gr in o.get('sharingGrantRefs',[]):
                g=ids.get(gr)
                if not g or g.get('grantState') != 'ACTIVE':
                    fail(f"{o['artifactId']} public disclosure references missing/non-active grant {gr}", errors)
        if 'use:partner-disclosure' in allowed or o.get('disclosureClass') == 'PARTNER':
            if not o.get('sharingGrantRefs'):
                fail(f"{o['artifactId']} partner disclosure lacks sharing grant", errors)
            for gr in o.get('sharingGrantRefs',[]):
                g=ids.get(gr)
                if not g or g.get('grantState') != 'ACTIVE':
                    fail(f"{o['artifactId']} partner disclosure references missing/non-active grant {gr}", errors)

    # Published/approved output-bearing records must use active output qualifications.
    def require_active_output(record, ref_key='outputQualificationRef'):
        oq=ids.get(record.get(ref_key))
        if not oq:
            fail(f"{record.get('artifactId')} missing output qualification {record.get(ref_key)}", errors); return
        if oq.get('qualificationState') != 'ACTIVE':
            fail(f"{record.get('artifactId')} references non-active output qualification {record.get(ref_key)}", errors)
    for a in by_type(records,'RegionalAlert'):
        if a.get('alertState') == 'PUBLISHED': require_active_output(a)
    for b in by_type(records,'BenchmarkDelta'):
        if b.get('benchmarkState') == 'PUBLISHED_ADVISORY': require_active_output(b)
    for p in by_type(records,'IntelligenceContributionPackage'):
        if p.get('packageState') in {'APPROVED_FOR_SHARE','SHARED','RECEIVED'}: require_active_output(p)
    for lp in by_type(records,'LearningArtifactSharePackage'):
        if lp.get('packageState') in {'APPROVED_FOR_SHARE','SHARED','RECEIVED'}: require_active_output(lp)
    for c in by_type(records,'FarmIntelligenceContribution'):
        if c.get('contributionState') in {'PACKAGED','SHARED','RECEIVED'} and c.get('outputQualificationRef'):
            require_active_output(c)

    # Regional alert -> risk signal -> source contribution chain hardening.
    for sig in by_type(records,'RegionalRiskSignal'):
        if sig.get('signalState') == 'ACTIVE':
            if not sig.get('sourceContributionRefs'):
                fail(f"{sig['artifactId']} active signal lacks source contributions", errors)
            for cref in sig.get('sourceContributionRefs',[]):
                c=ids.get(cref)
                if not c:
                    fail(f"{sig['artifactId']} references missing source contribution {cref}", errors)
                    continue
                if c.get('contributionState') in BAD_CONTRIBUTION_STATES:
                    fail(f"{sig['artifactId']} references unusable source contribution {cref}", errors)
                q=ids.get(c.get('qualityAssessmentRef'))
                if not q:
                    fail(f"{sig['artifactId']} source contribution {cref} lacks quality assessment", errors)
                else:
                    if q.get('assessmentState') != 'COMPLETE': fail(f"{sig['artifactId']} source contribution {cref} quality not complete", errors)
                    if q.get('anomalyClass') in {'HIGH','UNKNOWN'}: fail(f"{sig['artifactId']} source contribution {cref} high/unknown anomaly", errors)
                    if q.get('completenessClass') in {'INSUFFICIENT','UNKNOWN'}: fail(f"{sig['artifactId']} source contribution {cref} insufficient/unknown completeness", errors)
                    if q.get('freshnessClass') in {'STALE_BLOCKING','INVALIDATED','UNKNOWN'}: fail(f"{sig['artifactId']} source contribution {cref} stale/invalid/unknown freshness", errors)

    # Federated contribution quality/anomaly and training-use contribution eligibility.
    for fc in by_type(records,'FederatedLearningContribution'):
        if fc.get('contributionState') in {'SUBMITTED','ACCEPTED_BY_AGGREGATOR'}:
            qa=ids.get(fc.get('qualityAssessmentRef'))
            if not qa or qa.get('assessmentState') != 'COMPLETE' or qa.get('anomalyClass') in {'HIGH','UNKNOWN'} or qa.get('completenessClass') in {'INSUFFICIENT','UNKNOWN'} or qa.get('freshnessClass') in {'STALE_BLOCKING','INVALIDATED','UNKNOWN'}:
                fail(f"{fc['artifactId']} submitted/accepted federated contribution lacks clean complete quality assessment", errors)
            pr=ids.get(fc.get('poisoningOrAnomalyReviewRef'))
            if not pr or pr.get('reviewState') != 'COMPLETE' or pr.get('reviewDisposition') in BAD_POISON_DISPOSITIONS:
                fail(f"{fc['artifactId']} submitted/accepted federated contribution lacks nonblocking completed anomaly review", errors)
    for tr in by_type(records,'TrainingUseReceipt'):
        if tr.get('receiptState') == 'CONFIRMED' and tr.get('usedForTraining'):
            grant=ids.get(tr.get('grantCoverageRef'))
            if not grant or grant.get('grantState') != 'ACTIVE': fail(f"{tr['artifactId']} confirmed training use lacks active grant coverage", errors)
            for cref in tr.get('contributionRefs',[]):
                c=ids.get(cref)
                if not c: fail(f"{tr['artifactId']} references missing training contribution {cref}", errors); continue
                if c.get('artifactType') == 'FederatedLearningContribution' and c.get('contributionState') != 'ACCEPTED_BY_AGGREGATOR':
                    fail(f"{tr['artifactId']} training contribution {cref} not accepted by aggregator", errors)
                qa=ids.get(c.get('qualityAssessmentRef'))
                if qa and qa.get('anomalyClass') in {'HIGH','UNKNOWN'}: fail(f"{tr['artifactId']} uses high/unknown anomaly contribution {cref}", errors)

    # Cross-farm applicability high/local-use checks.
    for ass in by_type(records,'CrossFarmApplicabilityAssessment'):
        if ass.get('assessmentState') == 'COMPLETE' and ass.get('applicabilityClass') in {'MODERATE','HIGH'}:
            if not ass.get('similarityBasisRefs'):
                fail(f"{ass['artifactId']} moderate/high applicability lacks similarity basis", errors)
            if not ass.get('limitationRefs'):
                fail(f"{ass['artifactId']} moderate/high applicability lacks limitation disclosure", errors)
        if ass.get('assessmentState') != 'COMPLETE' and ass.get('applicabilityClass') in {'MODERATE','HIGH'}:
            fail(f"{ass['artifactId']} moderate/high applicability with non-complete assessment", errors)

    # Completed revocation traces must not coexist with active affected grants.
    for rv in by_type(records,'RevocationPropagationTrace'):
        if rv.get('revocationState') == 'COMPLETE':
            for gref in rv.get('affectedGrantRefs',[]):
                g=ids.get(gref)
                if g and g.get('grantState') == 'ACTIVE':
                    fail(f"{rv['artifactId']} completed revocation coexists with active affected grant {gref}", errors)

    # Alert corrections/withdrawals must target an existing alert.
    for corr in by_type(records,'RegionalAlertCorrection'):
        if corr.get('correctionState') in {'APPROVED','PUBLISHED'}:
            target=ids.get(corr.get('targetAlertRef'))
            if not target or target.get('artifactType') != 'RegionalAlert':
                fail(f"{corr['artifactId']} targets missing/non-alert record", errors)
    for wd in by_type(records,'RegionalAlertWithdrawal'):
        if wd.get('withdrawalState') in {'APPROVED','PUBLISHED'}:
            target=ids.get(wd.get('targetAlertRef'))
            if not target or target.get('artifactType') != 'RegionalAlert':
                fail(f"{wd['artifactId']} targets missing/non-alert record", errors)
...
def main():
    ...
    summary={
        ...
        'crossRecordPolicyDisclosureFederatedUseHardeningAware': True,
