...
def render_md(report: Dict[str, Any]) -> str:
    lines: List[str] = []
    lines.append('# OFARM economic intelligence real-farm pilot readiness report v0.2')
    lines.append('')
...
def output_paths(dataset: Path, out_dir: Optional[Path]) -> Tuple[Path, Path]:
    ...
    return target_dir / f'{base}_readiness_report.json', target_dir / f'{base}_readiness_report.md'


def main() -> None:
    parser = argparse.ArgumentParser(description='Validate readiness for bounded OFARM economics real-farm pilot lanes.')
