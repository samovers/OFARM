...
OUT_JSON = HERE / 'OFARM_economic_intelligence_real_farm_pilot_illustrative_results_v0_1.json'
OUT_MD = HERE / 'OFARM_economic_intelligence_real_farm_pilot_illustrative_summary_v0_1.md'
...
def lane_b(lane: Dict[str, Any]) -> Dict[str, Any]:
    ...
    options = [
        ...
        {
            'optionId': 'CONTRACTOR', 'label': contractor['label'],
...
def render_md(results: Dict[str, Any]) -> str:
    lines: List[str] = []
    lines.append('# OFARM economic intelligence real-farm pilot summary v0.1')
    lines.append('')
...
def main() -> None:
    ...
    out_json = path.with_name(path.stem + '_results.json') if path != DEFAULT_DATASET else OUT_JSON
    out_md = path.with_name(path.stem + '_summary.md') if path != DEFAULT_DATASET else OUT_MD
