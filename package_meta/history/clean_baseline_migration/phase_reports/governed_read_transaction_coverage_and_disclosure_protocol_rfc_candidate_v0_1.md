...
Date: 2026-09-14. Amended: 2026-09-17, Scope B version 2 deadline-failure projection alignment; Scope A unchanged.
...
Proposed change in plain English: consume PR #31's narrow permission for a historical read to expire because of identified database contention even after its evidence commits. Such an expiry can reveal hidden activity. It must preserve complete spent evidence, all read permissions, no stale disclosure and source protection until disclosure or irreversible termination. The existing positive case remains required when no covered wait lasts through the deadline; an expiry is a separate amended-privacy result, not that positive case passing.
...
7. Historical refusal reads consume PR #35's complete observation and original-admission verification in the same read context. Preserve its known-candidate refresh rule. For a policy-permitted limited reply with history WITHHELD, require the explicit source-backed sealing guarantee in §8.3 before final observation and preparation; hidden history alone is no post-commit cancellation condition. Only §8.3.1's evidenced covered wait through D may justify a hidden-induced success-versus-expiry difference, including after actual C. Independently required failures still stop disclosure. Deadline-failure messages on this path follow the separate uniform public selector in §8.3.1. Policy-selected exclusion, source commit ordering and history verification remain required.
...
5. **Policy-independent control, with Scope B's explicit deadline exception.** Whether protection is required, its scope, its policy and processing rules must not depend on hidden candidate presence, value, validity, count or classification. Protect the same candidate universe for equivalent requests. Only §8.3.1's import of PR #31 §5.4.1 permits a covered causal storage wait through D to change success into deadline failure, including after actual C. Optimistic conflict/deadlock aborts, arbitrary early timeouts, hidden-selected exclusion/readiness changes and unlisted processing failures remain outside that exception. A permitted timeout is an amended-privacy outcome, not a C38 pass under either stronger predecessor. Source commit ordering and all disclosure guards remain intact.
...
The corresponding no-hidden-writer execution may succeed. This explicitly permits a success-versus-deadline-failure privacy leak even when actual C preceded the hidden writer. Repeated reads may strengthen inference and no leakage bound is established; a uniform message does not remove it. Do not describe it as an independent fault, history-independent availability or conformance to either predecessor's stronger guarantee.
...
| RD-I13 | Scope B allows only the explicit covered-wait success-versus-failure difference at D, including identified database contention after actual C. Require actual RD-BIND03-W ordering through L/irrevocable termination, timely positive pairs absent covered waits, and uniform exclusion. Unlisted hidden-induced suppression/delay remains provider failure |
...
| RD-C16 C38 hidden-history pairs | For each independently allowed limited/excluded policy path, vary only hidden history, including NONE/disputed/unavailable. Public outputs match apart from Scope B's explicit covered-wait success-versus-deadline-failure distinction; exclusions always match. Include PR #31's C38 cross-expiry cases and same-independent-fault pair, with and without an additional covered wait: every deadline failure within that owner's selector uses identical HISTORICAL_READ_EXPIRED projection while internal causes, commitment knowledge and conformance findings remain distinct. No additional public content/category/diagnostic/hint/reference/omission difference. Include C20/C21, not just delivered-field comparisons | I09–I10, I13 |
