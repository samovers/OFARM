...
Date: 2026-09-14. Amended: 2026-09-17, Scope B version 2 early-failure and timing-cost alignment; Scope A unchanged.

Status: non-authoritative Phase A amendment draft to decision version 2; Scope B version 2 alignment is draft-authorized, not semantically approved. Exact-text review and renewed approval are required. No implementation or merge authorization.

Approval history: [version 2 approval](https://github.com/samovers/OFARM/pull/37#issuecomment-5662707529) covered `41cd45b90fb8e133f6377d8f389858b89c3dd7ae`; [renewed Scope A/B alignment approval](https://github.com/samovers/OFARM/pull/37#issuecomment-5687494237) covered `33abbe3c413a33ae38ba5ce189850c1d8556c4bd`. The task user's subsequent “do it” authorizes preparing the further Scope B amendment and this alignment, not accepting the eventual wording. Prior approvals do not transfer to amended bytes. Section 15.2 separates the owners; Scope A is unchanged.

Proposed change in plain English: consume PR #31's narrow permission for a historical read to expire because of identified database contention even after its evidence commits. The wait may prevent success or an independent error that the read would otherwise reach earlier; shorter waits may delay responses before the deadline. These differences can reveal hidden activity. Preserve immediate early failures, complete spent evidence, all read permissions, no stale disclosure and source protection until disclosure or irreversible termination. The positive case remains required when independent conditions permit disclosure and no covered wait lasts through the deadline; an expiry is a separate amended-privacy result, not that positive case passing.
...
7. Historical refusal reads consume PR #35's complete observation and original-admission verification in the same read context. Preserve its known-candidate refresh rule. For a policy-permitted limited reply with history WITHHELD, require the explicit source-backed sealing guarantee in §8.3 before final observation and preparation; hidden history alone is no post-commit cancellation condition. Only §8.3.1's evidenced covered wait through D may replace pre-D success or an independent terminal failure with expiry, including after actual C; that section also imports the limited pre-D timing cost. Independently required failures still stop disclosure immediately under §11. Deadline-failure messages on this path follow the separate uniform public selector in §8.3.1. Policy-selected exclusion, source commit ordering and history verification remain required.
...
| PR #35 §6 refresh versus PR #31 §5.4/C38 privacy | Refresh retained. Scope B imports only PR #31's proposed §5.4.1 deadline exception, now including identified database contention after actual C. Timely positive coverage, uniform exclusion and §8.3's source commit ordering remain required. Actual source/storage support remains RD-BIND03-W, a sub-obligation of open RD-BIND03 |
...
| History status/time may not be disclosed, and limited RECORDED_RESULT is permitted | WITHHELD/null/null remains fixed regardless of history. Before final S and sealing, require §8.3's actual source-backed ordering guarantee; only §8.3.1's covered wait through D permits hidden-induced expiry, including after C |
...
5. **Policy-independent control, with Scope B's explicit costs.** Whether protection is required, its scope, its policy and processing rules must not depend on hidden candidate presence, value, validity, count or classification. Protect the same candidate universe for equivalent requests. Only §8.3.1's import of PR #31 §5.4.1 permits a covered causal storage wait through D to replace pre-D success or an independent terminal failure with deadline failure, including after actual C, and the stated pre-D latency cost. Optimistic conflict/deadlock aborts, arbitrary early timeouts, hidden-selected exclusion/readiness changes and unlisted processing failures remain outside that exception. A permitted timeout is an amended-privacy outcome, not a C38 pass under either stronger predecessor. Source commit ordering and all disclosure guards remain intact.
...
For paired executions after **actual C, not its acknowledgement**, the unchanged limited reply must reach its one L in both executions under the same independent conditions permitting L, unless §8.3.1's covered wait remains outstanding through D. A newly offered hidden writer may cause the owner's specified database-internal contention and thus expiry before acknowledgement even after C; record that as the additional privacy tradeoff, not a successful positive pair. In a successful arm, prove the candidate's actual commit after L; in a covered-expiry arm, prove it after irrevocable termination. Rejecting or dropping the writer proves neither. This applies even with unresolved candidate admission/classification. The read never ignores an intervening commit, and genuine independent acknowledgement loss still suppresses L; no rule forces disclosure without acknowledged complete C.
...
PR #31's proposed §5.4.1 owns the sole exception. At the unchanged full D, a policy-permitted WITHHELD limited historical read may expire without L on that owner's closed list: relevant already-running source finalization/visibility/protection release; direct persistence/acknowledgement needed to establish read protection; the single C operation's atomic/durable completion and acknowledgement; and item 3's database-internal contention on those protection/finalization operations, including database completion/cleanup needed to issue acknowledgement after actual C. A newly offered hidden writer may cause item 3's wait. Identify the exact operation, contended internal resource and causal wait still outstanding through D; co-location or elapsed time alone proves nothing. This imports the owner's list without adding observation-query, processing or output-progress categories.

The corresponding no-hidden-writer execution may succeed or reach an independent terminal failure before D. The listed wait through D may prevent reaching that success or fault stage. PR #31 explicitly permits the resulting category/message/timing difference, including when actual C preceded the hidden writer. Hold the independent fault conditions fixed, retain the early arm's immediate existing failure projection and record a fault only where it actually occurred. Hidden activity cannot select among early failure categories or delay a failure already determined. Repeated reads, including ones encountering independent faults, may strengthen inference and no leakage bound is established; a uniform message does not remove it. Do not describe the difference as an independent fault, history-independent availability or conformance to either predecessor's stronger guarantee.

PR #31 also declares pre-D response latency caused by these listed storage dependencies as a privacy cost. If the wait ends and both arms finish before D, required public content still matches; timing may differ. No padding, extra failure category or delay after a terminal failure is determined is permitted. A wait ending before D but leaving too little subsequent scheduling time to meet D remains outside covered-expiry conformance.
...
Separately, PR #31 §5.4.1 requires the uniform HISTORICAL_READ_EXPIRED projection for every outward deadline failure on this independently policy-permitted WITHHELD historical path that reaches full D without L, whether a covered wait is present or absent. This includes independent acknowledgement failure alone or combined with covered hidden contention, and retained provider violations producing such a deadline failure. The public selector uses the verified path/deadline-failure boundary in §11, not cause, covered-wait proof, conformance status or commitment knowledge. The public owner defines all fields, labels, warnings and withholding; this protocol preserves truthful internal causes, known/unknown commitment, incident obligations and retry blocks. Uniform public wording neither grants another permitted wait nor certifies provider conformance. Earlier terminal failures keep their existing rules; delaying or buffering them until D cannot qualify them, and outward send time is not the boundary. Other paths, policy exclusions, fresh commands and status requests retain their existing projections. Apart from the declared pre-D outcome versus deadline-failure difference, hidden history cannot select extra public content, diagnostics, categories, hints, references or omissions.
...
RD-C20 separates timely positive progress from the owner's additional covered post-C expiry. Application pool acquisition, owner/controller/callback scheduling, network delivery and output-queue waits are not item 3's database contention. A database wait that ends before D but leaves too little time for subsequent scheduling also does not meet the wait-through-D condition. Such residuals, actual initiation, complete source participation and safe termination remain binding work; this proposal does not prove single-database feasibility or close #392/B1. Independent acknowledgement loss still stops disclosure, and any actual source commit inside the protected interval still fails provider conformance.
...
For C38 pairs with equivalent readable originals and differing only in hidden history, choose limited reply versus exclusion from the same independently selected reader/scope/record-class policy. Except for §8.3.1's expressly permitted pre-D outcome versus deadline-failure difference, hidden label, candidate count or observation success cannot change response category, text, diagnostics, references, hints, omission pattern or fallback choice. That section also imports the limited pre-D timing cost. This includes history learning around sealing, actual C and its acknowledgement, and L, not just fields in a delivered response. Sections 8.2–8.3.1 and RD-C20/RD-C21 supply the combined handling; the same coverage/egress guard applies to safe projections, headers, denial paths and caches.
...
Failure of any condition irrevocably suppresses this attempt; no protected bytes may cross L. A committed consumption remains spent even when L never occurs. Record that distinction truthfully. Scope B permits only §8.3.1's declared outcome and timing differences from the listed waits; its separate uniform deadline message does not certify §8.3's forbidden source interleaving or an unlisted hidden-induced delay. Those failures still violate C38/RD-C20 and block the binding. Do not trade stale disclosure for privacy or call expiry a conforming positive case. If L has already occurred, cancellation or later revocation cannot unsend bytes; do not claim zero disclosure. This protocol creates no later independent dispatch or filing transport attempt: `NO_EXTERNAL_DISPATCH` is unchanged, and #13/PR #11 §18.6 filing transport semantics are outside scope.

## 11. Failure, uncertain commitment and retry

Evidence commitment and disclosure are separate axes. Internal reconciliation distinguishes `EVIDENCE_COMMITTED`, `EVIDENCE_NOT_COMMITTED`, `EVIDENCE_COMMIT_UNKNOWN` and `EVIDENCE_INTEGRITY_BREACH`. Disclosure remains not attempted, observed attempted or unknown according to actual evidence, with any supported acceptance extent separately stated. These are runtime facts, not additions to the authorization lattice. The dispositions below remain internal facts and obligations even when §8.3.1 requires the same outward deadline-failure projection. That selector does not erase a cause, integrity breach, incident report or retry block, or postpone an earlier terminal failure until D.

For this selector, an **earlier terminal failure** is a failure determined by the trusted live owner before full D that requires it to irrevocably fence L; the owner must apply that fence immediately. The **deadline-failure boundary** is full D reached without L and without an earlier determined terminal failure. Bind the determination and effective fence to authoritative owner time/event evidence ordered against D, through the existing temporal/egress bindings; outward send time, a probe flag or a later log timestamp is not that proof. A delayed fence cannot turn an already determined early failure into deadline expiry. Reaching D prevents L even if processing resumes later. If boundary ordering or fence effectiveness is unproved, keep that fact unknown and fail closed under the applicable owner rules; do not invent a timely fence or a verified deadline classification. An armed downstream fault that the wait prevents reaching is not a determined failure. No new record family, clock, controller or fencing mechanism is selected here; the binding and its proof remain open.
...
| Covered pre-disclosure storage wait still outstanding at D | Only §8.3.1 permits limited WITHHELD pre-D success or an independent terminal failure versus deadline failure. Prove the exact causal wait, fence L and preserve actual commitment facts; do not invent a downstream fault prevented by that wait or label this a predecessor-privacy pass |
| Competing hidden qualifier while a permitted WITHHELD reply is sealed | Order its commit after L/irrevocable termination. With the same independent conditions permitting L and no covered wait through D, preserve the same limited reply. Only §8.3.1's identified database wait permits covered expiry after C. An intervening commit, pre-cut omission or unlisted hidden-induced delay still fails provider conformance; sanitized failure does not repair it |
...
| RD-I10 | PR #31 nulls/withholding/safe messages/C38 with only its declared pre-D outcome versus covered deadline failure and listed-wait pre-D latency costs; no extra hidden-selected public differences, and safe fallback does not prove producer readiness |
...
| RD-I13 | Scope B allows only §8.3.1's pre-D success or independent terminal failure versus covered deadline failure, including identified database contention after actual C, and its listed-wait pre-D latency cost. Require actual RD-BIND03-W ordering through L/irrevocable termination, timely positive pairs when independent conditions permit L and no covered wait lasts through D, and uniform exclusion. Unlisted hidden-induced suppression/delay remains provider failure |
...
| RD-C16 C38 hidden-history pairs | For each independently allowed limited/excluded policy path, vary only hidden history, including NONE/disputed/unavailable. Public outputs match apart from Scope B's declared pre-D outcome versus covered deadline failure and listed-wait pre-D latency costs; exclusions always match. Include PR #31's C38 early-failure pairs, latency variant, cross-expiry cases and silent independent-fault pair: every deadline failure within that owner's selector uses identical HISTORICAL_READ_EXPIRED projection while internal causes, commitment knowledge and conformance findings remain distinct. Earlier failures retain immediate original projections; no fabricated fault in a blocked arm or other public difference. Include C20/C21, not just delivered-field comparisons | I09–I10, I13 |
...
| RD-C20 Combined late hidden history and C38 | Pair same permitted original, authority/session/retention, deadline and independent faults, varying only a hidden candidate offered after actual C, including before acknowledgement and with unresolved admission. Timely pairs both reach L; the candidate commits afterward. Separately prove covered post-C contention expiry, spent C and candidate commit after irrevocable termination, using the variants below. An intervening commit or unlisted hidden-induced delay fails. Policy exclusions remain identical with no invented successful C/L. Retain C08's disclosable-history comparator | I02, I06–I10, I13 |
| RD-C21 Seal closure and admission adversaries | Candidate committed before acquisition but notified after C, omitted partition/import path, malformed/unresolved root, qualifier of qualifier, excluded current writer, forged/released seal, optimistic abort and hidden-dependent waits. Require complete pre-seal reconciliation and full protection. Timely acquisition retains the positive pair; only an evidenced §8.3.1 wait through D permits the declared pre-D outcome versus deadline-failure difference. Early timeout, artificial delay, history-classification difficulty, missing coverage/retention and arbitrary pool exhaustion are not covered. No notification blindness or empty-history fallback | I01–I03, I09–I13 |

For C20's limited-reply positive pair, observe **actual commit order, C acknowledgement and public output**, not merely whether the gate returns success. One execution has no competing candidate; the other attempts to commit it in the protected interval. With the same independent conditions permitting L and no covered wait through D, both disclose exactly once, and the latter candidate's actual commit is proved later than L. Test covered expiry separately below; it cannot replace this positive evidence. A candidate committing before L/irrevocable termination, or already committed before protection but omitted from final S, remains a provider failure even if admission is unresolved or the outward failure is sanitized.

Required post-C acknowledgement variants (specifications, not executed evidence):

| Case | Required result |
|---|---|
| Actual C commits; a newly offered hidden writer causes PR #31 §5.4.1 item 3's identified database-internal wait through D before acknowledgement; no-writer pair reaches L | Covered expiry under the proposed Scope B version 2 rule. Prove actual C separately from acknowledgement, the operation/resource and causal wait, no L, complete C still spent, irreversible termination before protection release, and the candidate's later actual commit after termination. Later acknowledgement never revives L; do not count this as the predecessor's positive pair passing |
| Compare the known-committed post-C expiry with RD-C16's pre-C/unknown-C expiries | Use PR #31's identical HISTORICAL_READ_EXPIRED projection while owner evidence preserves different commitment facts. No commitment-status signal through message, category, headers, hints, references or omissions |
| Same armed detectable acknowledgement-channel fault: no-writer arm reaches it after actual C before D; writer arm's identified item 3 wait through D prevents acknowledgement being issued and the fault firing. Also pair a post-acknowledgement L-guard/egress failure with an earlier covered wait that prevents reaching it | Immediate existing failure in the early arm versus HISTORICAL_READ_EXPIRED in the blocked arm is the declared cost. Prove §11's actual failure/fence ordering, actual C separately, no L and unchanged source ordering. Retain the applicable early commitment warning and truthful spent/uncertain facts; never record the untriggered fault as having occurred in the blocked arm |
| Hidden writer causes only callback/pool/network/output delay; the database wait ended before D; causation is unproved; or an early timeout/conflict/deadlock abort occurs | Outside the covered exception. No relabelling as storage progress or keeping an ended wait open in bookkeeping; fail closed without claiming covered-expiry conformance. A deadline failure within §8.3.1's separate public selector still uses its uniform message; an earlier terminal failure cannot be delayed to enter that selector |
...
| Same independent acknowledgement-delivery fault through D in both arms, actual C in both and no earlier terminal failure. Without hidden writer the database finishes; with hidden writer item 3's database contention also remains through D. Public response channel is healthy | Both produce identical HISTORICAL_READ_EXPIRED projections, including kinds, labels, warnings, withholding, references, omissions and carrier diagnostics. Preserve distinct actual causes and truthful commitment knowledge internally, all retry blocks and no late revival. Adding a covered wait cannot excuse an independent provider violation; uniform output cannot substitute for required positive progress |
...
RD-C16 also requires a definitive pre-C evidence-persistence fault reached immediately in the no-writer arm but prevented by an item 1 protection wait through D in the writer arm, using PR #31's declared early-failure-versus-expiry result. Its pre-D latency variant must prove the listed wait ended and both responses completed before D with matching required content. Across these cases, a failure determined before D but sent after D retains its early projection; a silent fault first terminating at D uses the deadline projection. Unsupported timing/fence claims fail binding verification rather than becoming invented deadline facts.
...
Version 2 was approved at `41cd45b90fb8e133f6377d8f389858b89c3dd7ae`; Scope A/B version 1 alignment was subsequently approved at `33abbe3c413a33ae38ba5ce189850c1d8556c4bd`. This further **Scope B version 2 alignment is draft-authorized only, not semantically approved**. Version 1 at `c2c52fa27bec24e72285564057cab198e4654e36` received the B1 request for changes, not semantic approval. Renewed owner review/approval must identify the revised exact bytes/head; a self-referential commit hash is not embedded in the candidate.

Renewed review covers only Scope B version 2's owner-derived changes and affected invariants/cases, not unchanged Scope A, artifact-content, selected-path, source-history or identity decisions. Approval would not certify the missing bindings in §14, including RD-BIND03-W, or grant another owner's authority. Distinguish timely positive progress, covered post-C database-contention expiry and retained provider violations. Static consistency is not proof that a provider can supply them.
...
Scope A and Scope B version 1 were drafted separately and later approved at their recorded heads. The current task-user instruction authorizes drafting Scope B version 2 and this exact consumer alignment. It creates no third owner or new prerequisite and does not approve the new semantics, implementation, provider admission, merge, promotion or extraction. Not provisional: no temporary implementation or waiver is introduced.
...
| B — confidentiality of hidden history in public read outcomes; PR #31 | `cp2_authorization_result_surface_and_public_reason_codes_rfc_candidate_v0_1.md` at `579e8def2dbd433500c53a30c5fc778353216694`, Scope B version 2, §5.4.1 and I04/C38 | §§8.3–8.4, 10–11; I10/I13; C16/C20/C21: closed causal waits through D including item 3 after actual C, separate uniform deadline-failure projection regardless of covered-wait presence, explicit pre-D success or independent terminal failure versus expiry and listed-wait pre-D latency costs, timely positive and separate expiry cases, unchanged source commit ordering | No late-C permission from B; no uniform-exclusion, disclosable-history, fresh-command, classifier or source-admission amendment |

The owner entries identify the unchanged [Scope A input](https://github.com/samovers/OFARM/commit/e9052efcf3c673360d939e856ac866cb27d709ae) and the new [Scope B version 2 proposal](https://github.com/samovers/OFARM/commit/579e8def2dbd433500c53a30c5fc778353216694), beyond the earlier pins in §3. The Scope B proposal is unapproved; review its exact changes together with this alignment before renewed semantic approval and later binding. Scope A is not reopened. Publication must verify the exact owner file against this pin; no old approval or rule digest silently changes. Shared D, C and L terminology does not transfer semantic ownership. Each owner can approve or amend its own text independently; this consumer cannot use approval of one scope to incorporate the other.
...
The initial 2026-09-17 Scope B version 2 alignment added the owner's narrow post-C database-contention exception. [PR #37 review 5236241276](https://github.com/samovers/OFARM/pull/37#pullrequestreview-5236241276) requested no separate correction at `004e724c32244ed9a5c4c2cdff284eba1a778dac`. [PR #31 review 5236241555](https://github.com/samovers/OFARM/pull/31#pullrequestreview-5236241555) then required separating public deadline messages from covered-wait eligibility. The task user's “go” authorizes this exact owner alignment and the same-independent-fault C16/C20 pair. No review clearance or semantic approval transfers to these corrected bytes. Scope A, the permitted wait list, source-withdrawal eligibility, source commit ordering, evidence membership, current authorization and spent-read rules stay unchanged. OFARM2 PR #396 remains paused; no adapter, storage topology or runtime mechanism is selected or proven.

The later [review of d1619af](https://github.com/samovers/OFARM/pull/37#issuecomment-5715589009) identified the owner's early-failure-versus-expiry omission and an undefined terminal boundary. The task user's “go” authorizes this bounded alignment with PR #31's explicit outcome/timing costs. Section 11 names the existing owner's determination/fence proof obligation; RD-C16/C20 cover definitive persistence, detectable acknowledgement and egress faults alongside the retained silent-fault pair. No new timing/fencing mechanism or permitted wait is added. Scope A and all source/evidence/disclosure guards remain intact; exact-head review and renewed semantic approval are still required.

What is next: review PR #31's Scope B version 2 proposal and this exact alignment, then obtain renewed exact-head semantic approval before using the weaker requirement. Keep #392/B1 and every actual binding, acceptance, conformance, promotion, extraction, implementation and merge gate open.
