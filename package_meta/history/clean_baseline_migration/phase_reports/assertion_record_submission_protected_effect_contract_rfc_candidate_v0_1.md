...
Date: 2026-09-03<br>
Status: Phase A delivery/conformance sequencing amendment candidate for `samovers/OFARM#22`; changed sequencing pending renewed exact-head review and approval; non-authoritative, not accepted law, and not a current/default machine contract<br>
Parent inventory: `samovers/OFARM#12`<br>
Depends on: compatible revised PR #11 authorization and PR #26 NOT_REQUIRED transaction candidates, pending review as specified in section 3; the approved PR #20 human-finalization input at `98f8c4fafbae42c8f7fd931f43f53adcb4733713` remains unchanged. Historical starting pins are retained below.<br>
Blocking prerequisite: PR #20 covers fresh approval and direct human final action; PR #26 owns the NOT_REQUIRED transaction profile. `ASSERT_OPERATION_CLAIM` remains non-executable until compatible reviewed owner contracts and actual immutable profile bindings close that mode.<br>
...
records that existing decision and its limits. Its approved source/checker bytes
retain their historical pending labels unchanged. This does not approve the
corrected owner heads or authorize implementation or merge.

Sequencing amendment proposal, 2026-09-24, based on exact owner head `a0a06ad99516790d5b908d5de330591691dd0dcd`: only section 18's delivery/conformance authorization is reopened for renewed exact-head review and approval. Historical exact-head approvals retain their original scope; earlier pending labels and review provenance are not new requests to reopen unchanged assertion mappings, branches, actor/time rules, classifications or postconditions. No historical approval covers this new construction allowance. This amendment is unapproved and grants no current implementation, merge, extraction, promotion or production permission. The primary boundary of this amendment is delivery/conformance authorization; the assertion-domain and shared-evidence owners remain unchanged.
...
The historical PR #11 input at `03a21f669ee04f96d444e14f00ae7212cab04803` defines authorization for assertion submission and binds the whole effect intent. It does not authorize acceptance or define this result. Its later approved release-scope head `4494924998183fe3fa7bc1b63b76a85893335044` and Scope A read-settlement head `e9052efcf3c673360d939e856ac866cb27d709ae` preserved the operation-claim rule before the checkpoint change. Those compatible historical changes do not approve the new timing closure or its owner revisions.
...
`6ec9f6650c3848083742c7039ac7a673b4375c8b`. These are review provenance, not
executable bindings or approval of this correction. Review and any later approval
must name the corrected triple together; historical pins are not silently replaced.
...
Here D means PR #26 section 8.3's full unchanged `decisionValidUntil` minimum,
including the original `transactionDeadline`. That owner requires T < D,
then permits the same fixed original transaction to commit after D with
...
owner constructs and verifies checkpoint evidence afterward. `PC_TEMPORAL_SEPARATION`
checks the proposed AssertionRecord and its already available inputs before T.
PR #26 sections 9.4 and 13.3 separately own verification of the later consumption,
attempt and receipt members, including their physical-time-field prohibition.
No future member or time must be invented to pass the domain validation.
...
3. the assertion's exact `/subject/subjectRef` occurs in the envelope's `/subjectRefs` array, or, only when no direct exact match exists, the selected body schema supplies an immutable relationship proof that makes the association explicit;
...
Direct subject comparison precedes the relationship-proof alternative. If one or more `/subjectRefs` entries match exactly, direct membership is used, the lowest zero-based matching index is recorded, and the proof alternative is not selected. Only when no direct subject match exists may the relationship-proof branch pass; on that branch the direct-match index is absent and the exact immutable proof binding is recorded. Direct anchor membership similarly selects and records the lowest zero-based `/anchorScopes` index among all exact pair matches. Duplicate exact subject or anchor entries create no additional semantic effects, and the bound full-envelope digest continues to cover every duplicate entry.
...
| `PC_TEMPORAL_SEPARATION` | the proposed AssertionRecord preserves exact governed assertedAt and subjectTime from its available inputs without substituting evidence/capture, ingress, approval, authorization, checkpoint or physical time; the selected branch adds no checkpoint or physical commit/consumption field to this result. This pre-T domain check does not validate later transaction evidence members; PR #26 sections 9.4/13.3 own them |
...
| selected operation-claim success members omit physical commit/consumption time while their owner bindings contain matching T, boundary, original cutoffs and attempt | PR #26 sections 9.4/13.3 verify those later transaction members, admissibility and authoritative commit status; the earlier PC_TEMPORAL_SEPARATION trace checks only its domain result and available inputs |
| a later consumption, attempt or receipt member in that branch labels T as physical commit/consumption time, or adds a null/provisional physical time | PR #26 section 9.4 verification fails; stop before success, or quarantine an already committed breach under its sections 10.1/11.3; no backdating or post-hash repair, and no invented domain postcondition result |
...
| schema-valid envelope contains duplicate exact subject and/or anchor entries | direct membership selects the lowest zero-based matching index, duplicates create no additional semantic effects, and every conforming implementation produces the same validation-trace bytes and digest; any other selection or trace fails conformance |
...
| When a semantic event is bound, must its immutable envelope and separate Event Ingress classification pass exact `/primaryEventFamily`, `/subjectRefs`, `/anchorScopes`, time, and dominant-context compatibility, with the lowest zero-based direct-match indices or a proof only when no direct subject match exists, and enter only the trace? | yes | yes |
...
7. **Bundle binding and accepted semantic promotion:** separately bind the exact reviewed protected-effect contract identity/ref/digest as one unit into `AuthorizationPolicyBundle v0.2`. Complete the selected manifest/dependency binding review and the separate accepted-semantic promotion required by PR #11 section 24 before construction under step 8. This does not promote machine contracts to current/default or dispose of missing three-assertion input, shared-evidence, temporal or identity dependencies by implication.
8. **Conformance construction:** only after step 7 and under every condition in PR #11 section 24.2, separately approve each affected runtime boundary's Phase A design and the exact test-only candidate-consumption mechanism. Those approvals may permit isolated construction of actual assertion application, protected-effect validation and persistence paths against the exact reviewed candidate package. All domain mappings, branch restrictions and foreign proof obligations remain unchanged; another owner's promoted-input requirement, including step 2 where applicable, is not waived.
9. **Hostile conformance:** exercise section 13 through those production-reachable paths, including variable event families, delayed assertion/receipt/commit time, subject-time substitution, cross-context/self/future correction targets, multi-context single-anchor representation, fresh approval, agent-assisted submission, represented action, reporter/performer separation, and conflicting replay. Record the exact package, implementation revision, configuration and observed outcomes; no successful dependency stub or helper-only run substitutes for required conformance.
10. **Current/default promotion:** change current/default status only through the separate steward-approved steps in the sequence fixed by PR #11, after the required hostile conformance and review. Earlier accepted-semantic promotion does not imply this step.
11. **OFARM2 production extraction:** consume only promoted, digest-verified canonical bytes in production. Step 8's separately approved isolated test-only consumption does not provide production admission.
12. **Separate production admission:** require the exact tested implementation and promoted package to pass the existing release decision after the preceding gates. No stage automatically resumes a task, merges a PR, deploys code or changes an assertion's truth/currentness. Changed package, implementation or configuration inputs require affected verification under PR #11 section 24.2. OFARM2 #392/#396 remains parked, and this allowance closes no remaining owner dependency.
