...
def main() -> int:
    ...
    if "package_meta/history/clean_baseline_migration/phase_reports/" not in dev_excludes:
        issues.append("DEV profile does not exclude package_meta/history/clean_baseline_migration/phase_reports/")
    if "OFARM2_*.zip at repo root" not in dev_excludes:
        issues.append("DEV profile does not exclude root OFARM2 ZIP artifacts")
    if "package_meta/history/clean_baseline_migration/phase_reports/" not in audit_includes:
        issues.append("AUDIT profile does not include package_meta/history/clean_baseline_migration/phase_reports/")
    ...
    for rel in audit.get("requiredFolders", []):
        if rel == "package_meta/history/clean_baseline_migration/phase_reports/":
            continue
        if not (REPO / rel.rstrip("/")).is_dir():
    ...
    try:
        readiness = load_json("package_meta/release/final_clean_baseline/CLEAN_BASELINE_FINAL_PACKAGE_READINESS.json")
        validation = load_json("package_meta/release/final_clean_baseline/CLEAN_BASELINE_FINAL_VALIDATION_LEDGER.json")
        debt = load_json("package_meta/release/final_clean_baseline/CLEAN_BASELINE_UNRESOLVED_DEBT_REGISTER.json")
    except Exception as exc:
