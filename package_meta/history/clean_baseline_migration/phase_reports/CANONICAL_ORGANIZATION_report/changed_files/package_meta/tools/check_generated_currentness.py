...
def main() -> int:
    ...
    for path, record in records_by_path.items():
        if (path == "README.md" or path.endswith("/README.md")) and record.get("activeLaw") is True:
            issues.append(f"README metadata is file-level active law: {path}")
        if (path == "folder.status.json" or path.endswith("/folder.status.json")) and record.get("activeLaw") is True:
            issues.append(f"folder.status metadata is file-level active law: {path}")
        if ("/current/" in path or "/currentness/" in path) and record.get("activeLaw") is True:
            issues.append(f"current reader/currentness view is file-level active law: {path}")
        if path.startswith("package_meta/generated/") and record.get("activeLaw") is True:
            issues.append(f"generated view is file-level active law: {path}")
        if (
            ...
            issues.append(f"clean-baseline proof/release metadata is file-level active law: {path}")
    ...
    source_map = load("package_meta/history/clean_baseline_migration/ledgers/CLEAN_BASELINE_GENERATED_VIEW_SOURCE_MAP.json")
