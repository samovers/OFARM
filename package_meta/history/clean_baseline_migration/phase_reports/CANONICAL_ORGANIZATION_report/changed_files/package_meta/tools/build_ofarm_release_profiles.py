...
def profile_excluded(profile_id: str, rel: str) -> bool:
    if common_excluded(rel):
        return True
    if profile_id == "DEV" and rel.startswith("package_meta/history/clean_baseline_migration/phase_reports/"):
        return True
    return False
...
def check_required(profile: dict, stage: Path, profile_id: str) -> list[str]:
    ...
    for rel in profile.get("requiredFolders", []):
        if profile_id == "DEV" and rel == "package_meta/history/clean_baseline_migration/phase_reports/":
            continue
        if not (stage / rel.rstrip("/")).is_dir():
            issues.append(f"missing required folder: {rel}")
    phase_reports = stage / "package_meta/history/clean_baseline_migration/phase_reports"
    if profile_id == "DEV" and phase_reports.exists():
        issues.append("DEV staging unexpectedly contains package_meta/history/clean_baseline_migration/phase_reports/")
    if profile_id == "AUDIT" and not phase_reports.is_dir():
        issues.append("AUDIT staging does not contain package_meta/history/clean_baseline_migration/phase_reports/")
