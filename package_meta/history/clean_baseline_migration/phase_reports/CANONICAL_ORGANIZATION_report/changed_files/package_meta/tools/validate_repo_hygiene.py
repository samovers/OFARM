...
def excluded(rel: str) -> bool:
    ...
    return (
        ...
        or rel.startswith("package_meta/history/clean_baseline_migration/phase_reports/")
...
def main() -> int:
    ...
    root_clean_baseline_clutter = [
        p.name for p in REPO.iterdir()
        if p.is_file()
        and (
            p.name.startswith("CLEAN_BASELINE_")
            or p.name.startswith("DEV_AUDIT_RELEASE_PROFILE_SPLIT")
            or p.name == "FINAL_CLEAN_BASELINE_AUDIT_PROMPT.md"
        )
    ]
    if root_clean_baseline_clutter:
        issues.append(f"root clean-baseline provenance clutter present: {sorted(root_clean_baseline_clutter)}")
    ...
    for path, record in material_by_path.items():
    ...
    for rel in current_files:
        if not (rel == "folder.status.json" or rel.endswith("/folder.status.json")):
            continue
        try:
            status_data = load(rel)
        except Exception:
            continue
        if status_data.get("activeLaw") is True:
            issues.append(f"folder.status.json uses activeLaw=true without artifact-level separation: {rel}")
        if status_data.get("artifactRole") == "folder_status_metadata" and status_data.get("metadataArtifactActiveLaw") is not False:
            issues.append(f"folder.status metadata does not declare metadataArtifactActiveLaw=false: {rel}")

    with (REPO / "MATERIAL_STATUS.csv").open(newline="", encoding="utf-8") as f:
        csv_rows = list(csv.DictReader(f))
        csv_paths = {r["path"] for r in csv_rows}
    if csv_paths != expected_status_files:
        issues.append("MATERIAL_STATUS.csv path set does not match current package tree")
    for row in csv_rows:
        path = row.get("path", "")
        active = str(row.get("activeLaw", "")).lower() == "true"
        if (path == "README.md" or path.endswith("/README.md")) and active:
            issues.append(f"README metadata is file-level active law in MATERIAL_STATUS.csv: {path}")
        if (path == "folder.status.json" or path.endswith("/folder.status.json")) and active:
            issues.append(f"folder.status metadata is file-level active law in MATERIAL_STATUS.csv: {path}")
        if ("/current/" in path or "/currentness/" in path) and active:
            issues.append(f"current reader/currentness view is file-level active law in MATERIAL_STATUS.csv: {path}")
        if path.startswith("package_meta/generated/") and active:
            issues.append(f"generated view is file-level active law in MATERIAL_STATUS.csv: {path}")
        if (
            path.startswith("package_meta/history/clean_baseline_migration/")
            or path.startswith("package_meta/release/final_clean_baseline/")
        ) and active:
            issues.append(f"clean-baseline proof/release metadata is file-level active law in MATERIAL_STATUS.csv: {path}")
    ...
    if not any(p.startswith("package_meta/history/clean_baseline_migration/") for p in indexed):
        issues.append("PACKAGE_META_INDEX.json does not index clean-baseline migration history")
