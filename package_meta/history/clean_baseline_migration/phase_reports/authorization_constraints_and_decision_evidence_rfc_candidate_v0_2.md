...
Aligned: 2026-09-23, approved PR #39 decision version 3 — selected operation-claim write checkpoint; owner revision pending review
Status: Phase A delivery/conformance sequencing amendment candidate for `samovers/OFARM#10`; earlier amendment provenance remains `samovers/OFARM#18`; changed sequencing pending renewed exact-head review and approval; non-authoritative, not accepted law, and not a current/default machine contract
Triggered by: review of `samovers/OFARM2#353` and draft `samovers/OFARM2#359`
Blocks: production admission of `samovers/OFARM2#353` and draft `samovers/OFARM2#359` until accepted semantics, successful conformance, promoted contracts, verified production extraction and separate release approval exist; conformance-only construction may be authorized solely through section 24, not by this candidate or its presence
...
Subsequent [Scope A semantic approval](https://github.com/samovers/OFARM/pull/11#issuecomment-5687492775)
applies to exact head `e9052efcf3c673360d939e856ac866cb27d709ae`. Earlier pending
labels are historical; the read decision is not being reopened. Section 18.5.1
and the human-finalization/filing protocols remain unchanged. This write-timing
owner revision still requires review and approval of its changed bytes.

Sequencing amendment proposal, 2026-09-24, based on exact owner head `2b46dbed647981a55704fd69fe06b92b7240be37`: only the delivery/conformance authorization in section 24 and its necessary restriction cross-references are reopened for renewed exact-head review and approval. Historical exact-head approvals retain their original scope; earlier pending labels and review provenance are not new requests to reopen unchanged authorization, release-scope, read-settlement or write-timing meaning. No historical approval covers this new construction allowance. This amendment is unapproved and grants no current implementation, merge, extraction, promotion or production permission. The primary boundary of this amendment is delivery/conformance authorization; each actual runtime change remains separately owned.
...
### 1.3 Selected write-timing alignment

This revision applies task-approved `OFARM-NOT-REQUIRED-WRITE-BOUNDARY-001`
version 3 at [PR #39, `6ec9f6650c3848083742c7039ac7a673b4375c8b`](https://github.com/samovers/OFARM/pull/39/commits/6ec9f6650c3848083742c7039ac7a673b4375c8b),
starting from this owner's `e9052efcf3c673360d939e856ac866cb27d709ae`.
Only explicitly bound `NOT_REQUIRED` `ASSERT_OPERATION_CLAIM` changes its temporal
authorization boundary to section 18.4.1's guarded checkpoint. The matching
transaction and protected-effect revisions belong to PR #26 (starting `38af747`)
and PR #23 (starting `622376e`). This is the authorization owner's part of that
write-timing/evidence boundary, not a change to Scope A, source consent, authority
paths, human finalization or read disclosure. The approved semantic choice permits
...
The twenty rows in section 7.2 remain the complete proposed action catalogue, subject to the explicit timing amendments in sections 1.2 and 1.3. Catalogue membership is not executable admission. A promoted authorization-policy package has an explicit, immutable admitted-action set in its existing binding manifest. Its policy identity and digest bind that set and exactly one complete resolved rule for each member. Missing, duplicate, extra or invalid members make the package inadmissible; a runtime cannot repair it by selecting a subset.

The initial release scope contains exactly `ASSERT_OPERATION_CLAIM` and `RECEIVE_READ_DATA`. Both retain their full resource alternatives, source-path rules, declarative projections and evidence requirements. Their timing meanings must be bound exactly: section 18.4.1 defines the explicitly selected write checkpoint, while section 18.5.1 independently defines Scope A read settlement. Neither exception follows from release membership alone. This scope does not restrict `RP_READ_TARGET_ONE` to operation claims or add a receipt target. A narrower public operation is governed by its own admitted operation binding; it cannot alter authorization-rule meaning or activate unrelated endpoints.
...
The operation-claim row's TX token admits the section 18.4.1 meaning only through
that exact revised rule/profile binding. No other TX or NOT_REQUIRED row inherits
the write checkpoint; the read row retains its separately bound section 18.5.1 meaning.
...
- `decisionValidUntil`: the unchanged exclusive full cutoff; it limits the selected write checkpoint in section 18.4.1, while governed reads have only section 18.5.1's evidence-settlement exception, never a later disclosure cutoff;
- `writeAuthorizationCheckedAt`, only for section 18.4.1: T observed at the final guarded checkpoint, never physical commit or consumption time;
- `effectCommittedAt`: actual commitment time, absent from every atomic success member for the section 18.4.1 attempt and optional only in truthful later owner evidence for that branch; and
...
`subjectTime` never substitutes for `authorizationEvaluatedAt`, `writeAuthorizationCheckedAt`, `effectCommittedAt`, or `effectDispatchedAt`.
...
Instants are compared as UTC timeline instants and all ends are exclusive. An explicitly unbounded governed interval contributes no cutoff; a missing or unparseable cutoff that its governing contract requires is non-`ALLOW`. If the candidate set cannot be constructed, or its minimum is not later than `authorizationEvaluatedAt`, no consumable decision exists. The decision must be consumed inside that protected-effect transaction and is not portable to a later transaction. **D is this complete minimum, equal to `decisionValidUntil`**, including the original transaction deadline. No universal duration is invented. The deadline limits T rather than physical transaction lifetime only for the explicitly selected section 18.4.1 branch. For governed reads, actual finalization initiation and protected disclosure must both occur strictly before D; only the already-initiated evidence operation may settle later under section 18.5.1. All other branches retain their consumption-at-commit cutoff.
...
The consumption evidence records the decision, effect-intent and derived-view digests, consumer principal, branch-correct time evidence, effect/read/outbox reference, and idempotency key where applicable. For section 18.4.1 it binds T, boundary, original cutoffs and original attempt before hashing; physical commit/consumption time for this attempt is absent from every atomic success member, not null, provisional or a renamed T. Actual complete commit still establishes durable consumption, proved by authoritative status and matching membership. Other write branches retain consumption time. For governed reads, preparation/initiation facts and the transaction owner's actual settlement facts remain distinct under section 18.5.1. A second consumption is non-`ALLOW` even if the payload is identical. For formal filing, expiry constrains the human-final outbox commit; separately eligible later transport neither consumes filing authority again nor inherits disclosure permission.
...
### 18.4.1 Explicit operation-claim write checkpoint

Only the exact rule/profile selected in section 7.4 for `NOT_REQUIRED`
`ASSERT_OPERATION_CLAIM` may use this branch. Its sole result is one immutable
`PENDING_REVIEW` assertion. The PR #26 transaction owner places the trusted final
checkpoint after its section 9.2 step 10 fixes the result binding and before step
11 hashes success evidence. All success-authorizing domain decisions, exact
decision/intent/result bytes and evidence inputs are fixed; guards and uniqueness
are rechecked. The original transaction samples trusted T and requires **T < full D**.
Missing clock trust or original-context mismatch prevents success.
...
original cutoffs and original attempt before hashes. The label matches the exact
rule. PR #26 retains its consumption → attempt → receipt digest order and complete
matching-set verification. No atomic success member claims this attempt's physical
commit/consumption time. Earlier input/assertion timestamps retain their meanings;
optional later physical-time evidence points back truthfully without rewriting
the original receipt. T does not itself consume or reserve durable authority.

Rollback, failed-checkpoint recovery, changed fixed inputs or replaced context
cannot reuse T; a fresh admitted attempt needs fresh authorization and T under
the existing outcome, durable-evidence and retry rules. Separately authorized
abort paths, including every PR #26 section 11.5 withdrawal condition and its
original first-dispatch cutoff, remain unchanged; T neither closes nor extends
that window or binds its missing prerequisites. No clock budget, mechanism,
other write, human finalization, filing, read or accepted-force permission changes.
...
19. Payload substitution and replay are non-`ALLOW`. Expiry prevents consumption except for the explicitly selected fixed original write transaction with T < full D under section 18.4.1, and the single read-evidence operation initiated before D under section 18.5.1. The former permits late durable write consumption; the latter only spends the read attempt and never permits late disclosure. Other branches retain their consumption-at-commit cutoff.
...
| An otherwise eligible claim write or governed read enters a complete admitted two-action package | the exact bound selected rule can reach authority-gate `ALLOW`; the separately owned transaction/disclosure gates must also pass before an effect or release; blanket refusal is not completion |
...
This table does not authorize an OFARM2 fix. Conformance-only construction requires every condition in section 24.2 and its separately approved implementation design. Production admission still requires accepted semantics, successful conformance, promoted contracts, verified production extraction and separate release approval.
...
8. **Conformance construction:** only after step 7's complete selected binding review and accepted semantic promotion, and only under all conditions in section 24.2, separately approve each affected runtime boundary's Phase A design and the exact test-only consumption mechanism. Those approvals may permit isolated construction of actual evaluator, application and persistence paths against the exact reviewed candidate package. They do not admit that package or implementation into production.
9. **Hostile conformance:** a separate PR exercises the production-reachable cases in section 22 through those actual implementation paths for the complete selected scope, including positive coverage and excluded-action admission failures, and publishes evidence identifying the exact package, implementation and configuration. Unselected action-specific cases remain deferred, not passed. Helpers or successful dependency stubs cannot replace required paths or proofs.
10. **Explicit scoped promotion:** after hostile review and steward approval, a separate PR updates current/default indexes and generated navigation for the exact reviewed package variant. It must prove that unrelated actions or omitted profiles do not become current/executable through family-level selection. Full-catalogue promotion is a later, separately approved change.
11. **OFARM2 production extraction:** promoted canonical assets are copied byte-for-byte and verified by digest and provenance for production consumption. Step 8's isolated test-only consumption is not this production extraction or permission for normal runtime selection.
12. **Separate production admission:** only after the preceding gates may the exact tested implementation and promoted package be considered for a separately reviewed release. Neither stage completion nor this source amendment automatically resumes an issue, merges a PR or deploys code. Authority evaluation, protected-effect validation, buffered disclosure, filing-outbox commitment and transport-release enforcement retain their separately reviewed runtime trust boundaries. No dispatcher may infer release eligibility from an authorization decision or outbox row.
...
### 24.2 Conditions for conformance-only construction

This is a narrow proposed change to construction and test-consumption permission, not to any action's authorization, domain or transaction semantics. It takes effect only through renewed exact-head approval and the governed accepted-semantic step above; this candidate alone supplies no permission. All of the following conditions are mandatory:

1. **Closed reviewed inputs first.** The complete selected manifest, real schema/contract bytes, immutable digests and dependency closure must have passed the existing binding review and accepted semantic promotion. The admitted set remains exactly `ASSERT_OPERATION_CLAIM` and `RECEIVE_READ_DATA`, with all selected target/path/evidence obligations; the full catalogue remains unchanged. No placeholder binding, unavailable required producer or successful stub may be counted as closure.
2. **Separate implementation approval.** Each affected runtime boundary requires its own explicit approval of a Phase A design under OFARM2's existing instructions before implementation. The design must include the precise conformance scope and the candidate-consumption mechanism, its authority and isolation, the immutable input bindings, real entry points and persistence paths, falsifiable controls and evidence. An invalidated design stops for an approved amendment. Approval of this sequencing text is not approval of that design or mechanism.
3. **Test-only consumption.** The separately reviewed mechanism verifies the exact reviewed candidate manifest, bytes, digests and provenance and rejects missing, altered or unreviewed inputs. It must not make candidate contracts selectable by normal production configuration, introduce an unspecified bypass flag or weaker alternate policy, or relax authorization, database or readiness protections. Construction uses isolated branches/worktrees, disposable test resources and fictional data; it must not use production credentials/data or expose a public endpoint. Test artifacts and results are conformance evidence only, not production authority, admitted domain truth or production-readiness claims.
4. **Actual paths and truthful evidence.** Conformance must exercise the actual evaluator/application/persistence paths through the intended entry points, including required successful actions, refusals, failures, races and recovery. A missing foreign dependency remains missing; no successful placeholder can stand in for its producer. Evidence identifies the exact candidate package, implementation revision, configuration, path and observed result. Section 22's real-path requirement is unchanged.
5. **Release remains separate.** Successful construction or testing does not merge, deploy, promote, change current/default selection or admit production use. Hostile review, steward approval, current/default promotion, verified production extraction and the separate release decision remain mandatory. Changed package, implementation or configuration inputs invalidate affected evidence and require the corresponding checks again; evidence reused for unchanged tested inputs must retain its exact provenance and documented applicability. This creates no cross-digest source-consent compatibility.
6. **Owning restrictions remain binding.** This allowance does not close shared trace/protected-effect/input bindings, source-history completeness, temporal qualifier/time trust, twin applicability, custody, read/write provider or other required owner obligations. Section 24.1 remains unchanged and still blocks complete binding while its proof is absent. If another owner prohibits the required construction or requires promoted assets, stop that affected work for that owner's explicit compatible disposition; do not infer an override. OFARM2 #392/#396 remains parked; this amendment and stage completion do not automatically resume either work item or any other runtime task.

---

## 25. Steward approval card

Before accepted-law work begins, stewards must be able to trace explicit decisions for every applicable item. Existing decisions remain at their approved exact heads; this amendment requests renewed approval only for the changed delivery/conformance sequencing:

The questions below retain the earlier approval record and unchanged criteria. Their historical approval labels are not new requests to reopen unchanged semantics through this amendment. Unresolved owner approvals remain unresolved; this amendment grants none. Review section 24's changed delivery permission, including section 24.2, at the exact proposed head. Historical approval remains evidence only at its named head, and a scope-model approval is not a finding that the section 24.1 history dependency is closed.
...
| Is the exact twenty-action catalogue, including delegation, CP3 posture, resources, subjects and existence postures, distinct from executable release scope? | yes; catalogue membership alone admits nothing, and sections 1.2/1.3 timing amendments need exact rule bindings | renewed approval for this distinction and revised bindings |
...
| Must all twelve delivery stages apply to the complete selected scope, with construction permitted only under section 24.2 and production admission retained as a separate decision, while the other eighteen actions and unrelated profiles remain explicit deferred obligations, not passed work? | yes; the full programme remains open | renewed exact-head approval required for the changed sequencing only |
...
- the twenty-action catalogue and exact initial two-action admission are distinguished, with the explicit timing amendments, complete selected-rule closure, excluded-action behavior and exact scoped promotion requirements reviewed;
...
What is next: review the exact changed sequencing bytes and compatible PR #26/#23 staging amendments for renewed approval. Existing exact-head approvals of unchanged semantics retain their scope. Keep history, temporal/twin/provider and all other owner dependencies open; no runtime design, test-only consumption mechanism, merge, currentness change or production release is approved by this candidate.
