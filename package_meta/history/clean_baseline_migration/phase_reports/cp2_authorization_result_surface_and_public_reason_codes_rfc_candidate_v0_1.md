...
Date: 2026-09-07. Amended: 2026-09-15, Scope B version 1 draft.

Status: non-authoritative Phase A amendment candidate; Scope B drafting authorized, including its explicit deadline-based privacy downgrade; revised wording pending exact-text review and renewed semantic approval.

Approval history: [Phase A version 1 approval](https://github.com/samovers/OFARM/pull/31#issuecomment-5624225401) applies to predecessor head `092be94f3a67497ba619295932cd0b2b1e9443f3`. The task user's subsequent “approve both” authorizes drafting Scope B version 1 here and Scope A version 1 with its separate owner, not acceptance of the eventual wording. Earlier review records below remain history; approval does not transfer to amended bytes. Scope A's late-evidence settlement permission is not incorporated in this public-privacy owner draft.
...
2. The single complete read-evidence finalization operation: storage completion needed for its actual atomic/durable C and the owner's authoritative acknowledgement of that exact membership. This does not permit starting or retrying finalization after its governing cutoff, and is subject to the actual-C post-commit restriction below.

The binding must identify the exact participating storage operation and demonstrate the causal wait through D. “Storage progress” is not a catch-all for history discovery or classification difficulty, incomplete source coverage, missing retention capability, arbitrary pool exhaustion, serializer/processing failure or unrelated resource problems. Those conditions retain their own owner rules; they do not qualify under this exception. Any broader availability exception needs a separately stated semantic change, not a new interpretation of this list.

The matched execution without the hidden writer may succeed before D. That success-versus-expiry difference is an explicitly permitted availability disclosure under this amendment. It may reveal hidden activity or contention even when no valid qualifier ultimately commits; repeated separately requested reads may strengthen the inference. No statistical bound on this leakage is established. Constant messages and null history fields do not eliminate it. Do not label the difference history-independent, an independent fault or a pass of the predecessor's stronger privacy requirement.
...
Use only the existing independently permitted safe failure surface, with truthful confirmation status and unchanged registered mappings. Do not expose the hidden writer/root/candidate, class, count, source path, lock dependency or a new explanatory hint. No history-dependent extra distinction among failure categories or diagnostics is authorized. A timeout or cancellation request does not prove rollback; an actually committed candidate or C must remain committed and be reconciled truthfully. This public exception alone grants no late-C settlement permission.

**The post-C positive obligation starts at actual C, before its acknowledgement.** Holding independent faults and all other conditions permitting L fixed, a hidden candidate newly offered between actual C and L must not prevent the same permitted limited reply in the paired execution, including by delaying C's acknowledgement beyond D. Its actual commit must be ordered after L. Such hidden-only acknowledgement delay, suppression, or an actual candidate commit in the protected interval is provider failure under retained RD-C20, not this deadline exception; fail closed without calling suppression privacy conformance. Genuine independent acknowledgement loss still suppresses L and is not overridden to force a reply. Actual binding and proof of this remaining progress obligation stay open; Scope B does not close B1 or admit a PostgreSQL mechanism.
...
The shared remediation does not assert a pending commit. Section 6's mandatory COMMIT_UNCONFIRMED sentence carries that row's explicit pending-confirmation warning. RESULT_UNAVAILABLE retains its settled relevant-commit condition and does not acquire a new retry permission from the wording change.
...
| CP2A-I04 | Honest permission/absence/trace qualification and hidden-independent policy selection/content, except only section 5.4.1's explicit limited-read success-versus-deadline-failure privacy downgrade; uniform exclusion and actual-C post-commit protection remain unchanged |
...
All positive COMMITTED_REFUSAL cases require the source-history qualification under section 5.4; a fresh commit does not waive that field. C01-C04 state their valid owner determination and required observation in each row rather than deriving NONE from a commit. Their future source fixtures must bind the real CP2A-DEP01 contract before they count as producer/adapter conformance evidence. C31-C39 add corrected, restricted, unavailable, invalid-shape, root-field and paired-fallback variants. Original outcome, root asOf, registered mappings and retry restrictions remain fixed across those variants.
...
| A covered finalization or direct protection-storage operation remains incomplete at D | No L; exact causal wait and truthful commit uncertainty required, without assuming rollback or late-settlement permission |
...
| C actually commits; before acknowledgement reaches the original owner, a newly offered hidden candidate delays acknowledgement past D | Provider failure under RD-C20, not a permitted deadline exception. Fail closed; suppression is not privacy conformance |
| Candidate offered after actual C, with the same independent conditions allowing L | Both paired limited replies reach L; candidate actually commits after L, not merely rejected or dropped |
...
Existing decision ID: `OFARM-ISSUE30-CP2-AUTHORIZATION-RESULT-PUBLIC-REASONS-001`, version `1`, approved only at the predecessor head recorded above. Scope B version 1 is authorization to draft an amendment to that public-privacy decision, not semantic approval of the revised candidate. Renewed approval must identify the exact reviewed revision and section 5.4.1's explicit privacy cost and retained obligations. It does not supply merge authority, admit bytes, select current/default contracts or approve implementation. CP2A-DEP01 and OFARM2 #392's actual mechanism/progress proof remain open; this amendment creates no additional approval scope or prerequisite issue.
...
What is next: review the Scope B amendment at the exact revised text/head, including the closed waits, explicit privacy cost, actual-C acknowledgement race and uniform exclusion. Obtain renewed semantic approval before separately authorized materialization; keep CP2A-DEP01, actual bindings and implementation gates open.
