...
Date: 2026-09-07. Amended: 2026-09-17, Scope B version 2 with explicit early-failure and timing costs.
...
Proposed change in plain English: a permitted historical read may time out at its unchanged deadline because of identified database contention, including contention after its complete evidence has committed. The wait can prevent either success or an independent error that the read would otherwise reach earlier. A wait that finishes earlier can also delay the response before the deadline. These differences may reveal hidden activity; a generic message does not hide them. Permissions, complete evidence, protection against stale disclosure and no reuse of a spent read remain required. This proposes a smaller requirement for review; it does not establish that one database can meet all the remaining requirements.
...
8. Scope B proposes only section 5.4.1's closed availability exception for independently permitted WITHHELD historical reads. A hidden writer's covered wait through D may replace an otherwise pre-D success or independent terminal failure with deadline failure; an earlier-ending listed wait may change pre-D response timing. These are explicit privacy costs, with immediate early-failure handling preserved. Separately, deadline failures on that same path use one proposed `HISTORICAL_READ_EXPIRED` response kind whether a covered wait is present or absent, reusing `AUTHORIZATION_RESULT_UNAVAILABLE` without reporting commitment or certifying provider conformance. This adds no permitted wait. Policy selection, public content apart from the declared outcome difference, actual-C-to-L protection and uniform exclusion remain protected. This is a semantic privacy downgrade, not conformance to the predecessor's stronger rule or a claim that OFARM2 #392 is implementation-ready.
...
| [PR #11](https://github.com/samovers/OFARM/pull/11), head `e9052efcf3c673360d939e856ac866cb27d709ae` | Sections 17.2, 18.2, 18.5–18.5.1, 18.7 and 24: immutable evidence with linked corrections/failures, full exclusive D, separately owned initiation/settlement/disclosure rules, authorized reads, committed refusal bundles, five public mappings and separate CP2 acceptance. Section 17.2 does not define the source-history classifier or its observation-point contract |
...
| [PR #26](https://github.com/samovers/OFARM/pull/26), head `38af7475d8cbd41b158e50ba77b60f140cbef4ba` | Sections 10–12 and 14: independently verified effect/evidence commits, terminal versus retryable consequences and authorized lookup. Section 11.5's conditional source-owned withdrawal remains unavailable pending its separate bindings |
...
The PR #11 and #26 pins were reconciled against the previously cited revisions. PR #11's five public mappings and PR #26's ordinary admission, evidence-profile and disclosure rules are unchanged. This public-projection amendment does not supply section 11.5's withdrawal projection, reader eligibility or other missing bindings, and does not authorize interruption or retry.
...
| `responseKind` | Required enum: `COMMITTED_REFUSAL`, `INGRESS_REJECTION`, `RESULT_UNAVAILABLE`, `COMMIT_UNCONFIRMED`, `HISTORICAL_READ_EXPIRED`. The last is legal only for section 5.4.1's deadline-failure projection on the independently permitted WITHHELD historical path, not selected by covered-wait presence |
...
The matched execution without the hidden writer may succeed or reach an independent terminal failure before D. A listed wait held through D may prevent the other execution reaching that success or fault stage, so it instead returns the uniform deadline failure. This explicitly permits differences in outcome category, the category's existing message/warning and response time, **including when C already committed before the hidden writer was offered**. The same independent fault conditions must be held fixed; record a fault only where it actually occurred, never fabricate it in the blocked execution. An early failure keeps its existing immediate handling and projection. Hidden activity cannot select among early failure categories or delay a failure already determined. The difference may reveal hidden activity or contention even when no valid qualifier ultimately commits; repeated separately requested reads, including ones encountering independent faults, may strengthen the inference. No statistical bound on this leakage is established. Constant messages and null history fields do not eliminate it. Do not label the difference history-independent, an independent fault or a pass of either predecessor's stronger privacy requirement.

Pre-D response latency caused by the same listed storage dependencies is also an explicit privacy cost: if the wait ends and both executions finish before D, their response times may differ. Their otherwise required public content must still match; this grants no extra early failure category, padding or delay after a terminal failure is determined. It does not permit expiry from a wait that ended before D but left insufficient subsequent scheduling time; that residual remains outside the wait-through-D exception above.
...
For outward deadline failures on this same independently permitted WITHHELD path, use the uniform public projection below independently of whether the failure qualifies for the closed availability exception. Existing failure mappings remain unchanged outside that projection's stated scope. Do not expose the hidden writer/root/candidate, class, count, source path, lock dependency or a new explanatory hint. Apart from the declared pre-D outcome versus deadline-failure difference, no history-dependent extra distinction among failure categories or diagnostics is authorized. A timeout or cancellation request does not prove rollback; an actually committed candidate or C must remain committed and be reconciled truthfully. This public rule grants no late-C settlement permission.
...
#### Uniform public projection for this path's deadline failures

The inspected CP2 carriers and this draft's four earlier response kinds do not supply this uniform deadline-failure projection with these confirmation limits. The proposed carrier remains one `HISTORICAL_READ_EXPIRED` value in this draft's existing AUTHORIZATION_RESULT branch, using the same proposed AUTHORIZATION_RESULT_UNAVAILABLE primary code. It adds no root/member field, top-level family or seventh reason code. This correction explicitly extends its applicability beyond covered waits on this same path; sections 6 and 8 bind that meaning. It is not a generic transport timeout or an already admitted contract.

Every outward deadline failure of an independently policy-permitted WITHHELD historical read that reaches its original full D without L must use this row. Select it from that path and the owner's verified deadline-failure boundary, **not** from the presence, absence or proof of a covered wait, a combination of faults, provider-conformance classification or whether C was initiated, settled or unconfirmed. Thus an independent acknowledgement failure through D and that same failure combined with hidden database contention have identical public projections. The owner must retain the distinct actual causes, commitment knowledge, retry blocks and any conformance violation internally; this public message establishes neither a covered cause nor provider conformance.

This projection does not extend the three permitted storage dependencies above. An unlisted hidden-induced failure or a forbidden source interleaving remains a provider failure even when its outward deadline message is uniform. Earlier terminal failures retain their existing rules: delaying or buffering an earlier failure until D does not make it a deadline failure, and the outward send time is not the selection boundary. Policy-excluded results, disclosable-history paths, fresh commands and current-operation-status requests remain outside this projection. It supplies no permission to delay failure, continue an invalid attempt, extend D or disclose protected information.

| Projection component | Required deadline-failure value or constraint |
|---|---|
...
For two deadline failures within this projection differing only in hidden activity, the response kinds, labels, warning sentences, redaction/absence posture, code definitions, trace fields, use qualifications, hints, reference exposure and omissions must match, including when only one has a covered wait and the same independent fault affects both. Apply the same rule to their carrier status, headers and other outward diagnostics; no generic transport fallback may restore the distinction. Only independently generated metadata already allowed by section 5.4 may differ, never values selected by cause, wait stage, conformance status or C knowledge. The projection cannot switch to a settled/unconfirmed message if more commitment facts arrive while forming the same expiry reply.
...
| Section 5.4.1's independently permitted WITHHELD historical read reaches full D without L and produces a deadline failure, whether a covered wait is present or absent; commitment and cause are not reported | HISTORICAL_READ_EXPIRED / null | AUTHORIZATION_RESULT_UNAVAILABLE | Historical read deadline reached |
| Outside section 5.4.1's deadline-failure projection: trusted runtime/persistence failure prevents returning a confirmed authorization result, with no relevant unresolved commit | RESULT_UNAVAILABLE / null | AUTHORIZATION_RESULT_UNAVAILABLE | Authorization result unavailable |
| Outside section 5.4.1's deadline-failure projection: any relevant effect or evidence commit needed for this current reply is unresolved | COMMIT_UNCONFIRMED / null | AUTHORIZATION_RESULT_UNAVAILABLE | Commit status unconfirmed |
...
Select section 5.4.1's deadline-failure row before the two generic failure rows, using its independently selected path and verified full-D failure boundary, never covered-wait proof, cause or commitment knowledge. Assess the closed availability exception and provider conformance separately. The public row reports neither a settled nor an unresolved outcome. Outside that row, unresolved relevant commit evidence takes precedence over presenting a local/prepared refusal or generic failure as settled. COMMIT_UNCONFIRMED is a public confirmation limit, not a rename or recalculation of PR #26's protected-effect outcome `OUTCOME_UNKNOWN`. A proven protected-effect rollback can coexist with an unresolved failure-evidence commit. The operation owner retains both facts; the public message still must not invite a new attempt while required evidence is unsettled.
...
- `developerMeaning`: `The qualified response reports a runtime or persistence failure, an unresolved relevant commit, or a deadline failure on the independently permitted WITHHELD historical-read path. responseKind distinguishes settled-result unavailability, unconfirmed commitment, and HISTORICAL_READ_EXPIRED, which reports neither commitment nor cause and does not establish provider conformance. None is an authorization outcome.`
...
| CP2A-I04 | Honest permission/absence/trace qualification and hidden-independent policy selection/content, except only section 5.4.1's limited-read pre-D success or independent terminal failure versus covered deadline failure, and its stated pre-D latency cost, including listed contention after actual C; uniform exclusion and source commit ordering through L/irrevocable termination remain unchanged |
...
| CP2A-C38 | Same authorized reader and equivalent readable originals; vary only hidden history or a competing hidden writer, holding independent authority, session, retention, deadline and fault conditions fixed | With timely covered progress, both permitted WITHHELD outputs match apart from the stated pre-D latency cost. Only section 5.4.1 permits pre-D success or independent terminal failure versus failure at D for an evidenced covered wait, including after actual C; do not call that a predecessor-privacy pass. Deadline failures on this path use its identical HISTORICAL_READ_EXPIRED projection even when only one has a covered wait or their internal commitment facts differ. Public uniformity does not establish provider conformance. Uniform exclusions still match. The variants retain source commit ordering and acknowledged complete C before L, and forbid public differences beyond the declared outcome and timing costs | I04, I05, I06, I07 |
...
All positive COMMITTED_REFUSAL cases require the source-history qualification under section 5.4; a fresh commit does not waive that field. C01-C04 state their valid owner determination and required observation in each row rather than deriving NONE from a commit. Their future source fixtures must bind the real CP2A-DEP01 contract before they count as producer/adapter conformance evidence. C31-C39 add corrected, restricted, unavailable, invalid-shape, root-field and paired-fallback variants. Whenever the original result is returned, its outcome and root asOf remain fixed. C38's deadline-failure row returns no original result and qualifies the current lookup failure instead. Every variant uses sections 6 and 8's exact governed mappings; retry restrictions remain unchanged.
...
| Same independent fault conditions: definitive evidence-persistence failure, detectable acknowledgement loss, or post-acknowledgement L-guard/egress failure. No-writer arm reaches the fault before D; writer arm's listed wait through D prevents reaching that stage | Early arm immediately uses its existing governed failure, including COMMIT_UNCONFIRMED's warning when applicable; blocked arm uses HISTORICAL_READ_EXPIRED at D. This category/message/timing difference is the declared cost, not a predecessor-privacy pass. No fabricated fault in the blocked arm, extra cause/stage hint, delayed early error, retry permission or altered source/C/L ordering |
| A listed storage wait ends before D; both arms subsequently finish before D with the same success or independent terminal failure | Timing may differ only as stated in section 5.4.1; required public content still matches. Record the actual wait and failure boundaries; no padding, early-category substitution or delay after a failure is determined. Separately reject covered-expiry conformance when the wait ended before D but scheduling then missed D |
...
| The same independent database-to-owner acknowledgement fault lasts through D in both executions; actual C occurs in both and neither terminates earlier. No hidden writer: database completion finishes but acknowledgement delivery is unavailable. Hidden writer: item 3's database contention also prevents issuing acknowledgement through D. Public response channel is healthy | Both return the identical HISTORICAL_READ_EXPIRED projection, not COMMIT_UNCONFIRMED in one arm. Match kinds, labels, warnings, withholding, codes, references, hints, omissions and carrier diagnostics. Keep the differing actual causes and truthful commitment knowledge internally, with all retry blocks and no late revival. Adding the covered wait does not excuse an independent provider violation |
| Any deadline failure within the projection above is instead RESULT_UNAVAILABLE or COMMIT_UNCONFIRMED; warnings/redaction vary; an earlier terminal failure is delayed until D or a current-status request is relabelled HISTORICAL_READ_EXPIRED | Reject the projection. The governing failure boundary, not send time, selects it. A uniform code or no protected bytes alone is insufficient. Outside its exact path/deadline scope, retain the original settled/unconfirmed meanings and mandatory confirmation warning |
| An unlisted hidden-induced wait or forbidden source interleaving produces a deadline failure on the same path | Use the uniform public projection while retaining the provider violation internally. The message cannot turn that execution into covered-wait conformance or replace required positive progress |
...
The same later authorization-branch/profile/fixture units must materialize section 5.4.1's single new response kind, its scope/precedence, fixed non-reporting warning, uniform withholding and C38 cross-expiry and same-independent-fault pairs, with the amended sixth entry's exact registry bytes. Keep the four earlier response-kind meanings and six legacy surface branches intact. No new artifact family, endpoint, schema file or implementation step is authorized by this correction.
...
The initial 2026-09-17 proposal added §5.4.1 item 3 and removed the blanket post-actual-C progress exception to Scope B. The [review of 5087253](https://github.com/samovers/OFARM/pull/31#pullrequestreview-5236241555) then found a failure-to-failure leak when covered contention combines with an independent acknowledgement fault. The task user's “go” authorizes this proposed correction: separate the public deadline-failure selector from the unchanged closed wait exception; add that fault pair and negative boundary cases to C38. Sections 5.2, 6, 8 and 9 reconcile applicability, including the sixth entry's developerMeaning; its other fields, the five other entries and all public warning bytes remain unchanged. No wider availability permission, storage mechanism or response family is proposed. PR #37 consumes the exact correction in its own boundary. This is not reviewer clearance or semantic approval; OFARM2 PR #396 remains paused and its provider unproved.

The later [review of dc88f93](https://github.com/samovers/OFARM/pull/31#issuecomment-5715552687) identified a distinct masked-fault pair: an otherwise early independent error versus expiry when a listed wait prevents reaching it. The task user's “go” authorizes declaring that cost and the pre-D latency posture in this owner draft, with the existing early-failure handling intact. C38 adds those variants; PR #37 defines the trusted failure boundary and aligns its cases. The uniform deadline projection, registry and warning bytes remain unchanged. This correction adds no wait, runtime mechanism or provider claim and requires renewed exact-head review and semantic approval.
