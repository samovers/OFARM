...
def main() -> int:
    ...
    for path, record in material_by_path.items():
        ...
        if path.startswith("CLEAN_BASELINE_") and record.get("activeLaw") is True:
            issues.append(f"clean-baseline ledger is file-level active law in MATERIAL_STATUS.json: {path}")
    ...
    for row in csv_rows:
        ...
        if path.startswith("CLEAN_BASELINE_") and active:
            issues.append(f"clean-baseline ledger is file-level active law in MATERIAL_STATUS.csv: {path}")
